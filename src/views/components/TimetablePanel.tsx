import ContentCopyIcon from '@mui/icons-material/ContentCopy';
import FileDownloadIcon from '@mui/icons-material/FileDownload';
import TerminalIcon from '@mui/icons-material/Terminal';
import WarningAmberIcon from '@mui/icons-material/WarningAmber';
import IconButton from '@mui/material/IconButton';
import Tooltip from '@mui/material/Tooltip';
import Typography from '@mui/material/Typography';
import { enqueueSnackbar } from 'notistack';
import React, { useCallback, useMemo, useRef } from 'react';
import {
  extractListMaLop,
  getTimetableSummary,
  getTongSoTcJudgement,
} from '../../utils';
import { selectActiveTimetableClasses, selectDataExcel, selectIsChiVeTkb, useTkbStore } from '../../zus';
import { getScriptDkhp } from '../3KetQua/utils';
import ThoiKhoaBieuTable, { TkbTableHandle } from './ThoiKhoaBieuTable';

type PanelState = 'no-file' | 'no-grid-selection' | 'no-manual-classes' | 'has-classes';

function getPanelState(hasFile: boolean, isChiVeTkb: boolean, activeClassesCount: number): PanelState {
  if (!hasFile) return 'no-file';
  if (activeClassesCount > 0) return 'has-classes';
  if (isChiVeTkb) return 'no-manual-classes';
  return 'no-grid-selection';
}

const EMPTY_STATE_MESSAGES: Record<Exclude<PanelState, 'has-classes'>, string> = {
  'no-file': 'Chưa có file',
  'no-grid-selection': 'Chưa chọn lớp',
  'no-manual-classes': 'Chưa có lớp',
};

type TimetablePanelProps = {
  compact?: boolean;
};

const TimetablePanel = React.forwardRef<HTMLDivElement, TimetablePanelProps>(function TimetablePanel(
  { compact = false },
  ref,
) {
  const tkbRef = useRef<TkbTableHandle>(null);
  const dataExcel = useTkbStore(selectDataExcel);
  const isChiVeTkb = useTkbStore(selectIsChiVeTkb);
  const activeClasses = useTkbStore(selectActiveTimetableClasses);

  const summary = useMemo(() => getTimetableSummary(activeClasses), [activeClasses]);
  const creditJudgement = useMemo(() => getTongSoTcJudgement(summary.tongSoTC), [summary.tongSoTC]);

  const listMaLop = useMemo(() => extractListMaLop(activeClasses), [activeClasses]);
  const script = useMemo(() => getScriptDkhp(listMaLop), [listMaLop]);
  const hasClasses = listMaLop.length > 0;

  const panelState = getPanelState(!!dataExcel, isChiVeTkb, activeClasses.length);

  const copyText = useCallback(async (text: string, successMessage: string) => {
    try {
      await navigator.clipboard.writeText(text);
      enqueueSnackbar(successMessage, { variant: 'success' });
    } catch {
      enqueueSnackbar('Không thể sao chép', { variant: 'error' });
    }
  }, []);

  const handleCopyMaLop = useCallback(() => {
    if (!hasClasses) return;
    void copyText(listMaLop.join(','), 'Đã sao chép mã lớp');
  }, [copyText, hasClasses, listMaLop]);

  const handleCopyScript = useCallback(() => {
    if (!hasClasses) return;
    void copyText(script, 'Đã sao chép script');
  }, [copyText, hasClasses, script]);

  const handleSaveImage = useCallback(() => {
    tkbRef.current?.saveTkbImage();
  }, []);

  return (
    <div ref={ref} className="schedule-panel timetable-panel">
      <div className="schedule-panel-header">
        <div className="timetable-panel-title-row">
          <Typography component="h2" className="subsection-title">
            Thời khóa biểu
          </Typography>
          {panelState === 'has-classes' && (
            <div className="timetable-panel-stats" aria-live="polite" aria-label="Thống kê lớp đã chọn">
              <span className="timetable-stat">
                <span className="timetable-stat-value">{listMaLop.length}</span>
                <span className="timetable-stat-label">Lớp đã chọn</span>
              </span>
              <Tooltip title={creditJudgement.text}>
                <span
                  className={'timetable-stat' + (creditJudgement.isOk ? '' : ' timetable-stat--warn')}
                  aria-label={`Số tín chỉ: ${summary.tongSoTC}. ${creditJudgement.text}`}
                >
                  {!creditJudgement.isOk && (
                    <WarningAmberIcon className="timetable-stat-icon" aria-hidden />
                  )}
                  <span className="timetable-stat-value">{summary.tongSoTC}</span>
                  <span className="timetable-stat-label">Số tín chỉ (14–24)</span>
                </span>
              </Tooltip>
            </div>
          )}
          {hasClasses && (
            <div className="timetable-panel-actions">
              <Tooltip title="Sao chép mã lớp">
                <IconButton
                  size="small"
                  className="timetable-action-btn"
                  onClick={handleCopyMaLop}
                  aria-label="Sao chép mã lớp"
                >
                  <ContentCopyIcon />
                </IconButton>
              </Tooltip>
              <Tooltip title="Sao chép script">
                <IconButton
                  size="small"
                  className="timetable-action-btn timetable-action-btn--primary"
                  onClick={handleCopyScript}
                  aria-label="Sao chép script"
                >
                  <TerminalIcon />
                </IconButton>
              </Tooltip>
            </div>
          )}
        </div>
      </div>

      {panelState === 'has-classes' ? (
        <>
          <div className="tkb-frame">
            <Tooltip title="Tải ảnh TKB">
              <IconButton
                size="small"
                className="timetable-action-btn tkb-download-btn"
                onClick={handleSaveImage}
                aria-label="Tải ảnh TKB"
              >
                <FileDownloadIcon />
              </IconButton>
            </Tooltip>
            <div id="thoi-khoa-bieu-wrapper">
              <ThoiKhoaBieuTable ref={tkbRef} compact={compact} />
            </div>
          </div>
        </>
      ) : (
        <div className="timetable-empty-state" role="status">
          {EMPTY_STATE_MESSAGES[panelState]}
        </div>
      )}
    </div>
  );
});

export default TimetablePanel;
